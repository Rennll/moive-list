...
function renderMovieList(data) {
  ...
  data.forEach(({ id, title, genres, description, release_date, image }) => {
    rawHTML += `
       <li class="list-group-item">
          <div class="row">
            <div class="col-8 align-self-center">${title}</div>
            <div class="col-4">
              <button class="btn btn-primary btn-show-movie" data-bs-toggle="modal" data-bs-target="#movie-modal" data-id="${id}">More</button>
...
// removeAndAddClass(Node Object, string, Node Object)
// 更新：將輸入值固定，而 lastTarget 可輸入可不輸入， return 則是為了 onPaginatorClicked(event) 使用
function removeAndAddClass(target, tagName, lastTarget = "") {
  if (!lastTarget) lastTarget = document.querySelector(`.${tagName}`);
...
axios.get(INDEX_URL).then((response) => {
  ...
  removeAndAddClass(document.querySelector("#card-display"), "actived-mod");
...
// 更新：利用 removeAndAddClass() ，將十一行減少為八行， return 回的兩個物件可更改其屬性與 innerHTML
paginator.addEventListener("click", function onPaginatorClicked(event) {
  if (event.target.tagName !== "A") return;
  const paginatorPage = removeAndAddClass(
    event.target.parentElement,
    "active",
    document.querySelector("#paginator .active")
  );

  if (paginatorPage.beforeNode) {
    paginatorPage.beforeNode.removeAttribute("aria-current");
    const actPage = paginatorPage.beforeNode.children[0].dataset.page;
    paginatorPage.beforeNode.innerHTML = `<a class="page-link" href="#" data-page="${actPage}">${actPage}</a>`;
  }

  page = Number(event.target.dataset.page);
  paginatorPage.afterNode.setAttribute("aria-current", "page");
  paginatorPage.afterNode.innerHTML = `<span class="page-link" data-page="${page}">${page}</span>`;
  renderPanelAndFavo(getMovieByPages(page));
});
